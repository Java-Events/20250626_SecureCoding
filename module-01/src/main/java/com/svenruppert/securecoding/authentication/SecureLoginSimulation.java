...
public class SecureLoginSimulation
  ...
  public static class Server
    ...
    private static final Map<char[], Login> LOGIN_DATABASE = new HashMap<>();
    private static final Map<char[], char[]> REGISTRATION_SALTS = new HashMap<>();
    private static final Map<char[], char[]> DYNAMIC_SALTS = new HashMap<>();
    ...
    public char[] requestDynamicSalt(char[] userName) {
      ...
      if (LOGIN_DATABASE.containsKey(userName)) {
        ...
        DYNAMIC_SALTS.put(userName, dynamicSaltForLogin);
    ...
    public char[] requestStaticSalt(char[] userName) {
      ...
      if (LOGIN_DATABASE.containsKey(userName)) {
        ...
        char[] staticSalt = LOGIN_DATABASE.get(userName).staticSalt();
        ...
        REGISTRATION_SALTS.put(userName, generatedSalt);
    ...
    public void registerUser(char[] userName, char[] hashed) {
      if (LOGIN_DATABASE.containsKey(userName)) {
      ...
      } else if (REGISTRATION_SALTS.containsKey(userName)) {
        ...
        Login login = new Login(
            userName,
            hashed,
            REGISTRATION_SALTS.get(userName));
        logger().info("userName2LoginMap.put {}", login);
        LOGIN_DATABASE.put(userName, login);
        logger().info("remove user {} from registrationSalts", userName);
        REGISTRATION_SALTS.remove(userName);
      } else {
    ...
    public boolean authenticate(char[] userName, char[] hashedPassword) {
      ...
      Login login = LOGIN_DATABASE.get(userName);
      if (login == null) {
        logger().info("User {} not found", userName);
        return false;
      }

      char[] dynamicSaltForLogin = DYNAMIC_SALTS.get(userName);
